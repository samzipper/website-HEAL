---
authors:
- jake-staab
bio: KGS Intern
education:
  courses:
  - course: B.S. in Geology (in progress)
    institution: University of Kansas
email: "jakestaab@ku.edu"
interests:
 []
name: Jake Staab
organizations:
- name: Kansas Geological Survey, University of Kansas
  url: "http://www.kgs.ku.edu/Hydro/hydroIndex.html"
- name: Department of Geology, University of Kansas
  url: "https://geo.ku.edu/"
role: KGS Intern
social:
- icon: envelope
  icon_pack: fas
  link: mailto:jakestaab@ku.edu
- icon: linkedin
  icon_pack: fab
  link: https://www.linkedin.com/in/jake-staab/
superuser: false
user_groups:
- Current Members
---
**Positions:**

 - [KGS Geohydrology Internship Program](https://kgs.ku.edu/geohydrology-internship-program), Summer 2026.
 - Undergraduate Research Assistant, Fall 2025-current.

**Supervisor:** [Bre Rivera Waterman]({{< ref "/authors/bre-rivera-waterman" >}}), [Archi Howlader]({{< ref "/authors/archi-howlader" >}}).

**Project:** Sustainable Adaptations to the Future Environment of Kansas' Agricultural And Water - SAFE KAW (funded by USDA).

**Favorite body of water:** Table Rock Lake, Missouri