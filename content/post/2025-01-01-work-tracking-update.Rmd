...
Targets for 2024 included:

  - Maintain number of weeks with 1 unplugged day (no work including checking email) and increase number of weeks with 2 unplugged days.
  - Decrease weeks where I work in the evening more than 1 day.
  - Keep percent of time spent on research at least 15%, with target of 20%.

## How I do it

I track the hours in a [simple google sheet](https://docs.google.com/spreadsheets/d/1Fg9g4pOHfuKfXeZ-mYbUPiZfkmrv0_dsZwzg8JbFpCA/edit?gid=1260315680#gid=1260315680) at 30 min resolution and divided into a few simple categories:
...
## Target 1: More unplugged days

Mostly a success across the board, with an overall increase in the number of unplugged days (from `r yearly_no_work$no_work_days_yr[yearly_no_work$Year == 2023]` in 2023 to `r yearly_no_work$no_work_days_yr[yearly_no_work$Year == 2024]` in 2024). At the weekly level, I increased the number of weeks with at least one unplugged day from `r yearly_no_work$weeks_gt_0[yearly_no_work$Year == 2023]` in 2023 to `r yearly_no_work$weeks_gt_0[yearly_no_work$Year == 2024]` in 2024 (approaching the theoretical maximum of 52) and had a marginal decrease in the number of weeks with 2 unplugged days (from `r yearly_no_work$weeks_gt_1[yearly_no_work$Year == 2023]` in 2023 to `r yearly_no_work$weeks_gt_1[yearly_no_work$Year == 2024]` in 2024).
...
## Target 2: Decrease evening work.

I had a backslide here, from `r evening_work_days$weeks_le_1[evening_work_days$Year == 2023]` weeks in 2023 with <= 1 day/week with evening work to `r evening_work_days$weeks_le_1[evening_work_days$Year == 2024]` weeks in 2024, matching where I was in both 2020 and 2022.
...
I mostly blame this on teaching an evening class during the fall 2024 semester, which led to a big chunk of hours.

```{r, echo=FALSE, fig.width=4, fig.height=9}
ggplot(evening_work, aes(x = YM, fill = Activity)) +
  geom_bar() +
  #facet_wrap(~Year, ncol = 1) +
  scale_y_continuous(name = "Half-Hours Worked After 6 PM",
                     expand = expansion(mult = c(0, 0.05))) +
  scale_x_date(name = "Month", limits = as.Date(date_limits),
               expand = expansion(0)) +
...
## Target 3: Research >15%, target 20%
...
Looking at some other major categories:
 ...
 - *Talking:* Made it to `r round(100*sum(work_by_year$Time_Prc[work_by_year$Year == 2024 & work_by_year$Activity == "talking"]), 1)`% in 2024, up slightly from `r round(100*sum(work_by_year$Time_Prc[work_by_year$Year == 2023 & work_by_year$Activity == "talking"]), 1)`% in 2023. This doesn't include meetings with mentees, which are counted separately.
 - *Mentees:* `r round(100*sum(work_by_year$Time_Prc[work_by_year$Year == 2024 & work_by_year$Activity == "mentees"]), 1)`% in 2024, unchanged from 2023. Looks like this has generally plateaued. With a couple postdocs and a grad student finishing in fall 2024, I'd project this to decrease next year.
 
Here's a look at the longer-term trajectories for these categories.

```{r, echo=FALSE, fig.width=8, fig.height=8}
work_by_month %>% 
  subset(Activity %in% c("research", "logistics", "mentees", "talking")) %>% 
  ggplot(aes(x = YM, y = Time_Prc, color = Activity)) +
  geom_point() +
  facet_wrap(~Activity) +
  scale_y_continuous(name = "Percent of time",
...
My 2025 goals are focused on maintaining and continued progress along most of these dimensions:

  - Maintain 1/day week with no work.
